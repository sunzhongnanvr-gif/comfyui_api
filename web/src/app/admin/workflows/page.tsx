  ...
  const handleImport = async (values: any) => {
    ...
    try {
      const submitData = {
        ...
        parameters: parsedParams, // 包含用户配置的参数可见性
      };
...
export default function WorkflowsPage() {
  ...
  const handleEditParams = async (id: string, name: string) => {
    ...
    try {
      ...
      if (data.success) {
        setEditParamParams(data.data.params || []);
      } else {
  ...
  const handleSaveParams = async () => {
    try {
      const data = await apiFetch(`/admin/workflows/${editParamWorkflowId}/params`, {
        method: 'PUT',
        body: JSON.stringify({ parameters: editParamParams }),
      });
      if (data.success) {
    ...
    <div>
      ...
      >
        <p style={{ color: '#666', marginBottom: 16 }}>
          ...
          <br />
          <Text type="secondary">勾选“可见”表示开放给用户；“激活/关闭”表示是否彻底跳过该参数。</Text>
        </p>
        <GroupedParamEditor
            ...
            onClick={async () => {
              try {
                await apiFetch(`/admin/workflows/${serverImportedId}`, {
                  method: 'PUT',
                  body: JSON.stringify({ parameters: serverParsedParams }),
                });
                message.success('参数配置已保存');
          ...
          <Upload
            ...
            beforeUpload={async (file) => {
              try {
                ...
                const data = await res.json();
                if (data.success) {
                  updateParam(index, { default: data.data.comfyui_filename });
                  message.success(`已上传: ${data.data.comfyui_filename}`);
                } else {
                  message.error(data.error || '上传失败');
                }
              } catch (err) {
  ...
  return (
    ...
    >
      <Alert
        ...
        description="配置哪些参数在客户端可见、哪些参数必填、修改显示名称和默认值。只有勾选必填的参数才会在测试和用户端被强制校验。"
        type="info"
        showIcon
